...
class PerformanceAnalytics:
    ...
    @staticmethod
    def generate_equity_chart(equity_curve, title="Equity Curve", save_path=None):
    ...
    @staticmethod
    def compare_strategies(results_dict, metric='total_return', title="Strategy Comparison"):
        """Compare multiple strategies based on a specific metric."""
        ...
        for strategy, value in zip(strategies, values):
            ...
            elif metric in ['avg_win', 'avg_loss', 'initial_equity', 'final_equity']:
                formatted_value = f"${value:.2f}"
            else:
                formatted_value = f"{value:.2f}"
            
            display_data.append([strategy, formatted_value])
        
        # Create table with tabulate
        table = tabulate(display_data, headers=["Strategy", metric.replace('_', ' ').title()], tablefmt="grid")
        
        return f"\n{title} (by {metric.replace('_', ' ').title()})\n{table}\n"
